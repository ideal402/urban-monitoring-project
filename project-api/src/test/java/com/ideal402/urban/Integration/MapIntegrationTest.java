...
class MapIntegrationTest {
    ...
    @Test
    @DisplayName("4. [POST /map/roads/traffic] H3 인덱스 목록으로 도로 트래픽 비동기 조회 시 200 OK 반환")
    void getRoadTrafficInArea_Success() {
        // Given
        List<String> h3Indices = List.of("8930e1d12abcdef", "8930e1d12abcdeg");

        when(roadTrafficService.getRoadTrafficByH3Indices(anyList()))
                .thenReturn(List.of(new RoadResponse("Test","Test","Test","Test",new BigDecimal("10"),"Test"))); // 실제 DTO 생성자나 빌더에 맞게 수정 필요

        // When
        String url = "http://localhost:" + port + "/map/roads/traffic";
        ResponseEntity<List<RoadResponse>> response = restTemplate.exchange(
                url,
                HttpMethod.POST,
                new HttpEntity<>(h3Indices), // Request Body에 H3 인덱스 리스트 포함
                new ParameterizedTypeReference<>() {}
        );

        // Then
        // TestRestTemplate은 내부적으로 비동기 응답(CompletableFuture)이 완료될 때까지 대기한 후 결과를 반환합니다.
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isNotNull();

        // Mock 객체가 비동기 스레드 내에서 정상적으로 호출되었는지 검증
        verify(roadTrafficService, times(1)).getRoadTrafficByH3Indices(h3Indices);
    }
